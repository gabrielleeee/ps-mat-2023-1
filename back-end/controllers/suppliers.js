...
controller.retrieve = async(req, res) => {
    try{
        const data = await Suppliers.findAll({
            include: {model: Products, as: 'products'} //include para aparecer se tem algum products ligado a ele
        }) 
...
controller.retrieveOne = async(req, res) => {
    try{
        const data = await Suppliers.findByPk(req.params.id) //findAll dá um select*
        //HTTP 200: OK (implícito)
...
controller.update = async (req, res) => {
    try{
        const response = await Suppliers.update(
            req.body,
...
controller.delete = async (req, res) => {
    try{
        const response = await Suppliers.destroy(
            { where: {id: req.params.id }}
        )
        if(response) {
            //Encontrou e excluiu
