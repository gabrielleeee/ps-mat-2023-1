...
controller.retrieve = async(req, res) => {
    try{
        const data = await Products.findAll({
            include: {model: Suppliers, as: 'supplier'} //include para aparecer se tem algum products ligado a ele
        }) 
...
controller.retrieveOne = async(req, res) => {
    try{
        const data = await Products.findByPk(req.params.id) //findAll dá um select*
        //HTTP 200: OK (implícito)
